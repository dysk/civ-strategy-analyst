...
A metric checkpoint that reports both `military_might` and
`military_units` also carries `might_per_unit`, the might an average unit
in that army is worth. `military_might` says how much force a
civilization can bring; `might_per_unit` says what that force is made of.
A large army of obsolete units and a small modern one can reach the same
total, and they are answered in completely different ways: the horde can
be held off by a handful of up-to-date defenders, while the modern stack
cannot be stopped by numbers.

Compare `might_per_unit` between civilizations at the same checkpoint,
...
strong military on its own. Read it alongside `military_might` and the
`military_might_swings` key moments: might that grew while the ratio held
steady is an army that got bigger, might that grew with the ratio is one
that was upgraded, and a ratio that climbed while might fell is an army
bled down to its best units.
