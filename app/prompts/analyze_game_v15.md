...
The `military_might_swings` key moments are measured on the raw
`military_might`, treasury included, so a swing there can be a purchase
or a windfall rather than a change in the army. Before calling one a
military build-up or collapse, check whether `army_power` moved with it
at the surrounding checkpoints; if it did not, the treasury moved and
the army did not.
