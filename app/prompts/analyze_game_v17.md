...
relations, empire geometry), the distances between the civilizations'
...
`ideology_adoptions`), reaching a new era ahead of the field
(`era_leads`), and completing a wonder a rival was plausibly also
building all belong in this class. Weigh them above small differences in
...
strongest beliefs are taken first, so an early pantheon compounds from
the turn it lands - read `lekmod.beliefs` for what was actually taken
rather than assuming the vanilla pick.

A handful of technologies open a window rather than adding an increment,
and `rush_tech_leads` reports who reached each one first, along with the
turn the next civilization followed where anyone did. Metallurgy is the
sharpest of them: it is the largest gain in siege power per beaker in the
game, and cities that held comfortably against crossbows and knights stop
holding, which is why so many mid-game snowballs begin there.
Industrialization matters for a different reason - it leads to factories
and to an ideology, and the civilization that adopts one first takes free
tenets while its rivals are still paying for theirs, a lead that widens
quietly for the rest of the game. Dynamite and Combustion open comparable
windows later, Flight and Radar later still; Machinery and Chivalry are
the earliest and the most situational of them.

A rush only pays if there is something to spend it on, so read every one
of these against `capital_proximity` and against how far the rival's own
cities reach. A civilization that arrived at one of these technologies far
ahead of the field, with a weaker neighbour close by, was handed a
decision - and whether it took that decision is usually among the most
revealing things in the game. The same lead with every rival far away
bought much less, because the window closes while the army is still
crossing the map. Say which of the two situations it was rather than
treating the lead alone as an advantage.

Two things this data cannot settle. Several of these units require a
strategic resource - Landships are worth nothing without oil - and the
digest records no resources at all, so never say a civilization held,
lacked or secured one, and never explain a rush that did not happen by a
resource it might not have had. And LEKMOD rebalances several of these
units, so take what each one does from `lekmod.general_rules` rather than
from the vanilla unit of that name; where the reference says nothing,
name the technology and leave its effect out.
...
are built once per civilization and race nobody. Credit a wonder that
matched what its builder was already doing and could afford at that
moment; treat one that plausibly displaced settlers, army or
infrastructure at a decisive moment as a cost. Holding more wonders than
a rival does not make a civilization stronger, and a wonder count must
never appear as evidence of a lead.
...
Where `geometry` describes the shape of one empire, the top-level
`capital_proximity` field describes the gaps between them. `capitals`
gives each civilization's first city and where it was founded, and
`distances` gives the hexes between every pair of capitals. Capitals are
founded on turn zero and never move, so these figures describe the
opening geography of the game and hold for its whole length, whatever
either side later conquered or lost.

Read them as reach. Two capitals a dozen hexes apart describe a game
where one civilization can pressure the other before either has much of
an army, and where a border dispute is close to inevitable; forty hexes
apart describes neighbours who have to cross a continent to touch each
other, so an army built early has nothing to do with them. A war between
distant capitals is a larger commitment than the same war between close
ones, and a civilization that stayed peaceful next to a weak neighbour
made a choice worth naming.

Two limits matter. The distance is measured capital to capital, not
between the nearest cities of two empires: borders can meet far from
either capital, so a large figure rules out early pressure on the
capital, not contact between the empires - check each civilization's
`span` before concluding two of them never met. And the distance is
straight-line hexes with no knowledge of what lies between, since the
digest carries no terrain: two capitals ten hexes apart may be separated
by ocean, mountains or a third civilization. Never describe a distance
as marchable, as a land route, or as an invasion path, and never convert
it into turns of travel.
...
of raised output without quoting percentages. A civilization running
negative happiness is draining its golden-age counter rather than filling
it, so long unhappy stretches and an absence of golden ages are usually
one story rather than two - say it once.
...
could have gone elsewhere. Where a `city_states` timeline shows a
civilization holding several allies across many turns, credit that as
real investment with real returns, and say what it plausibly cost. An
entry in `city_state_ally_takeovers` is a swing rather than a neutral
event - one civilization had paid for that ally and another took it, so
