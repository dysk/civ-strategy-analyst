...
class AnalyzeGameTest < ActiveSupport::TestCase
  ...
  test "sends the digest and versioned prompt to the injected llm client, and saves the report" do
    ...
    assert_equal File.read(Rails.root.join("app/prompts/analyze_game_v5.md")), stub.received[:system_prompt]
  ...
  test "persists a snapshot of the prompt used to generate the report" do
    ...
    assert_equal File.read(Rails.root.join("app/prompts/analyze_game_v5.md")), analysis.prompt
  end
